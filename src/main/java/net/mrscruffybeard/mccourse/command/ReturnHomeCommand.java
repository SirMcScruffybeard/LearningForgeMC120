package net.mrscruffybeard.mccourse.command;
...
public class ReturnHomeCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray(SetHomeCommand.HOME_POS).length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray(SetHomeCommand.HOME_POS);
